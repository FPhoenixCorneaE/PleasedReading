 ...
 * 上传管理的线程池
 */
public class OkUploadThreadPool {
    ...
    public OkThreadPoolExecutor getExecutor() {
        if (executor == null) {
            synchronized (OkUploadThreadPool.class) {
                if (executor == null) {
                    executor = new OkThreadPoolExecutor(corePoolSize, MAX_IMUM_POOL_SIZE, KEEP_ALIVE_TIME, UNIT, //
