...
public class OkHttpUtils {
     ...
     * 通用的拼接请求头
     */
    public static Request.Builder appendHeaders(Request.Builder builder, OkHttpHeaders headers) {
...
//                String headerValue = URLEncoder.encode(entry.getValue(), "UTF-8");
     ...
     * 生成类似表单的请求体
     */
    public static RequestBody generateMultipartRequestBody(OkHttpParams params, boolean isMultipart) {
        if (params.fileParamsMap.isEmpty() && !isMultipart) {
            ...
            //拼接文件
            for (Map.Entry<String, List<OkHttpParams.FileWrapper>> entry : params.fileParamsMap.entrySet()) {
                List<OkHttpParams.FileWrapper> fileValues = entry.getValue();
                for (OkHttpParams.FileWrapper fileWrapper : fileValues) {
    ...
    private static String getHeaderFileName(Response response) {
        String dispositionHeader = response.header(OkHttpHeaders.HEAD_KEY_CONTENT_DISPOSITION);
    ...
    public static MediaType guessMimeType(String fileName) {
        ...
        if (contentType == null) {
            return OkHttpParams.MEDIA_TYPE_STREAM;
        }
