 ...
 */
public class FRCardView extends FrameLayout {

    public static final int ADSORPTION = 0;
    public static final int LINEAR = 1;
    private int shadowFluidShape = ADSORPTION;
    private final int defaultShadowSize = 10;
    private final int defaultShadowStartAlpha = 10;
    private int elevation = 0;
    private int leftSize = defaultShadowSize,
            topSize = defaultShadowSize,
            rightSize = defaultShadowSize,
            bottomSize = defaultShadowSize;
    private int defaultShadowColor = 0x05000000;
    ...
    public FRCardView(@NonNull Context context) {
        this(context, null);
    }

    public FRCardView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FRCardView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        setWillNotDraw(false);
        int allShadowSize = -1;
        if (attrs != null) {
            TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.FRCardView, defStyleAttr, 0);
            int indexCount = typedArray.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = typedArray.getIndex(i);
                if (index == R.styleable.FRCardView_fr_cv_leftShadowWidth) {
                    leftSize = typedArray.getDimensionPixelSize(index, defaultShadowSize);
                } else if (index == R.styleable.FRCardView_fr_cv_topShadowHeight) {
                    topSize = typedArray.getDimensionPixelSize(index, defaultShadowSize);
                } else if (index == R.styleable.FRCardView_fr_cv_rightShadowWidth) {
                    rightSize = typedArray.getDimensionPixelSize(index, defaultShadowSize);
                } else if (index == R.styleable.FRCardView_fr_cv_bottomShadowHeight) {
                    bottomSize = typedArray.getDimensionPixelSize(index, defaultShadowSize);
                } else if (index == R.styleable.FRCardView_fr_cv_shadowSize) {
                    allShadowSize = typedArray.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.FRCardView_fr_cv_shadowColor) {
                    shadowColor = typedArray.getColor(index, defaultShadowColor);
                } else if (index == R.styleable.FRCardView_fr_cv_shadowStartAlpha) {
                    shadowAlpha = typedArray.getInt(index, defaultShadowStartAlpha);
                } else if (index == R.styleable.FRCardView_fr_cv_shadowFluidShape) {
                    shadowFluidShape = typedArray.getInt(index, ADSORPTION);
                } else if (index == R.styleable.FRCardView_fr_cv_cardBackgroundColor) {
                    cardBackgroundColor = typedArray.getColor(index, defaultCardBackgroundColor);
                } else if (index == R.styleable.FRCardView_fr_cv_cornerRadius) {
                    cornerRadius = typedArray.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.FRCardView_fr_cv_leftTopCornerRadius) {
                    leftTopCornerRadius = typedArray.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.FRCardView_fr_cv_leftBottomCornerRadius) {
                    leftBottomCornerRadius = typedArray.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.FRCardView_fr_cv_rightTopCornerRadius) {
                    rightTopCornerRadius = typedArray.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.FRCardView_fr_cv_rightBottomCornerRadius) {
                    rightBottomCornerRadius = typedArray.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.FRCardView_fr_cv_elevation) {
                    elevation = typedArray.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.FRCardView_fr_cv_elevationAffectShadowColor) {
                    elevationAffectShadowColor = typedArray.getBoolean(index, false);
                } else if (index == R.styleable.FRCardView_fr_cv_elevationAffectShadowSize) {
