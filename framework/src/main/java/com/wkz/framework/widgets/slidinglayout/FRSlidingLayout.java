...
/**
 * 实现类似微信、QQ 浏览器中网页的上拉下拉弹跳效果和 iOS 的 UITableView (ListView)的果冻效果。
 * 完美兼容 Android 自带库和兼容库的所有 View 组件，
 * 包括 RecyclerView、ListView、ScrollView 以及 WebView 等等，简单易用
 * github:https://github.com/HomHomLin/SlidingLayout
 * Created by Linhh on 16/4/12.
 */
public class FRSlidingLayout extends FrameLayout {
    ...
    public FRSlidingLayout(Context context) {
        this(context, null);
    }

    public FRSlidingLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FRSlidingLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.FRSlidingLayout);
        mBackgroundViewLayoutId = a.getResourceId(R.styleable.FRSlidingLayout_fr_sliding_background_view, mBackgroundViewLayoutId);
        mSlidingMode = a.getInteger(R.styleable.FRSlidingLayout_fr_sliding_mode, SLIDING_MODE_BOTH);
        mSlidingPointerMode = a.getInteger(R.styleable.FRSlidingLayout_fr_sliding_pointer_mode, SLIDING_POINTER_MODE_MORE);
        mSlidingTopMaxDistance = a.getDimensionPixelSize(R.styleable.FRSlidingLayout_fr_sliding_top_max, SLIDING_DISTANCE_UNDEFINED);
        a.recycle();
...
//        super.setOnTouchListener(l);
    ...
    public FRInstrument getInstrument() {
        return FRInstrument.getInstance();
    }
