 ...
 */
public abstract class CoolInternalBasePopup<T extends CoolInternalBasePopup<T>> extends CoolBaseDialog<T> {
    ...
    public CoolInternalBasePopup(Context context) {
        super(context);
        heightScale(1);
        dimEnabled(false);
    }
    ...
    public T location(int x, int y) {
        mX = x;
        mY = y - CoolStatusBarUtils.getHeight(mContext);
        return (T) this;
    }
}
