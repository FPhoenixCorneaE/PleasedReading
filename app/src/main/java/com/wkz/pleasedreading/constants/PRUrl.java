package com.wkz.pleasedreading.constants;

public class PRUrl {
    //一些数据API接口
    //https://blog.csdn.net/qq_21700489/article/details/78805245
    //聚合数据新闻
    public static final String BaseUrl_News = "https://www.juhe.cn/docs/api/id/235";
    //阿凡达图片
    public static final String BaseUrl_Images = "https://www.avatardata.cn/Docs/Api?id=ca48df82-cae4-401d-b0c1-af4c7a46edbe&detailId=e11078b9-8136-4f32-b6ee-ee7780e5f68e";

    //必应
    public static final String BaseUrl_BiYing = "http://cn.bing.com";
    public static final String BaseUrl_BiYing_Img = "http://s.cn.bing.net";
    //干货
    public static final String BaseUrl_Gank = "http://gank.io";
    //头条
    public static final String BaseUrl_TouTiao = "http://toutiao.com";
}
