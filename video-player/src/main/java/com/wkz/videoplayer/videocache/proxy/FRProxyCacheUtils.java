 ...
 */
public class FRProxyCacheUtils {
    ...
    static void assertBuffer(byte[] buffer, long offset, int length) {
        FRPreconditions.checkNotNull(buffer, "Buffer must be not null!");
        FRPreconditions.checkArgument(offset >= 0, "Data offset must be positive!");
        FRPreconditions.checkArgument(length >= 0 && length <= buffer.length, "Length must be in range [0..buffer.length]");
    }
    ...
    public static void close(Closeable closeable) {
        if (closeable != null) {
            try {
                ...
                FRVideoLogUtils.e("Error closing resource", e);
            }
        }
    }
