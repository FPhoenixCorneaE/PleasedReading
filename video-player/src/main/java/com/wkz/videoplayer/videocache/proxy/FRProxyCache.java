...
import com.wkz.videoplayer.videocache.source.FRSource;

import java.util.concurrent.atomic.AtomicInteger;

/**
 * Proxy for {@link FRSource} with caching support ({@link FRCache}).
 * <p/>
...
public class FRProxyCache {
    ...
    public FRProxyCache(FRSource source, FRCache cache) {
        this.source = FRPreconditions.checkNotNull(source);
        this.cache = FRPreconditions.checkNotNull(cache);
        this.readSourceErrorsCount = new AtomicInteger();
    }

    public int read(byte[] buffer, long offset, int length) throws FRProxyCacheException {
        FRProxyCacheUtils.assertBuffer(buffer, offset, length);
    ...
    private void checkReadSourceErrorsCount() throws FRProxyCacheException {
        int errorsCount = readSourceErrorsCount.get();
        if (errorsCount >= MAX_READ_SOURCE_ATTEMPTS) {
            readSourceErrorsCount.set(0);
            throw new FRProxyCacheException("Error reading source " + errorsCount + " times");
        }
    }

    public void shutdown() {
        synchronized (stopLock) {
            FRVideoLogUtils.d("Shutdown proxy for " + source);
            try {
                stopped = true;
    ...
    private synchronized void readSourceAsync() throws FRProxyCacheException {
        boolean readingInProgress = sourceReaderThread != null && sourceReaderThread.getState() != Thread.State.TERMINATED;
        if (!stopped && !cache.isCompleted() && !readingInProgress) {
            sourceReaderThread = new Thread(new SourceReaderRunnable(), "FRSource reader for " + source);
            sourceReaderThread.start();
        }
    }

    private void waitForSourceData() throws FRProxyCacheException {
        synchronized (wc) {
            try {
                wc.wait(1000);
            } catch (InterruptedException e) {
                throw new FRProxyCacheException("Waiting source data is interrupted!", e);
            }
        }
    }
    ...
    private void tryComplete() throws FRProxyCacheException {
        synchronized (stopLock) {
    ...
    private void closeSource() {
        try {
            ...
            onError(new FRProxyCacheException("Error closing source " + source, e));
        }
    }

    protected final void onError(final Throwable e) {
        boolean interruption = e instanceof FRInterruptedProxyCacheException;
        if (interruption) {
            FRVideoLogUtils.d("FRProxyCache is interrupted");
        } else {
            FRVideoLogUtils.e("FRProxyCache error", e);
        }
    }
