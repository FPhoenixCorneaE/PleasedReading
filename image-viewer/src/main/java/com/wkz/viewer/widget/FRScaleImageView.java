...
import com.wkz.viewer.dragger.FRDefaultFRImageDragger;
import com.wkz.viewer.dragger.FRImageDragger;
import com.wkz.viewer.dragger.FRImageDraggerState;
import com.wkz.viewer.dragger.FRImageDraggerType;
import com.wkz.viewer.dragger.FRWXImageDragger;
import com.wkz.viewer.dragger.OnImageDraggerStateListener;
import com.wkz.viewer.widget.progressbar.FRCircleProgressBar;

/**
 * 可缩放图片的自定义 View（即 viewPager 的 item）
 */
public class FRScaleImageView extends FrameLayout {
    // 图片的位置
    private int mPosition;
    // view 的相关数据
    private FRViewData mViewData;
    // 动画执行时间
    private int mDuration;
    // 默认的预览界面的宽高
    private float mDefWidth, mDefHeight;
    // 图片拖拽处理类
    private FRImageDragger mFRImageDragger;
    // 图片拖拽模式
    ...
    // 图片拖拽状态监听
    private OnImageDraggerStateListener mStateListener;


    public FRScaleImageView(@NonNull Context context) {
        super(context);
        initView(context);
    }

    public FRScaleImageView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public FRScaleImageView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context);
    }

    private void initView(Context context) {
        mDuration = FRImageViewerAttacher.DEF_DURATION;
    ...
    private void initDragStateMonitor() {
        mStateListener = new OnImageDraggerStateListener() {
            @Override
            public void onImageDraggerState(int state) {
                switch (state) {
                    case FRImageDraggerState.DRAG_STATE_READY:
                        isImageDragging = true;
                        break;
                    case FRImageDraggerState.DRAG_STATE_DRAGGING:
                        isImageDragging = true;
                        break;
                    case FRImageDraggerState.DRAG_STATE_BEGIN_REBACK:
                        isImageAnimRunning = true;
                        isImageDragging = false;
                        break;
                    case FRImageDraggerState.DRAG_STATE_REBACKING:
                        break;
                    case FRImageDraggerState.DRAG_STATE_END_REBACK:
                        isImageAnimRunning = false;
                        break;
                    case FRImageDraggerState.DRAG_STATE_BEGIN_EXIT:
                        isImageAnimRunning = true;
                        isImageDragging = false;
                        break;
                    case FRImageDraggerState.DRAG_STATE_EXITTING:
                        break;
                    case FRImageDraggerState.DRAG_STATE_END_EXIT:
                        isImageAnimRunning = false;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction() & ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                 ...
                 * 3、拖拽处理类不为空
                 */
                if (ev.getPointerCount() == 1 && getScale() <= 1f && mFRImageDragger != null) {
                    float diffX = ev.getX() - mDownX;
                    float diffY = ev.getY() - mDownY;
                    // 上下滑动手势
                    if (Math.abs(diffX) < Math.abs(diffY)) {
                        if ((mDragType == FRImageDraggerType.DRAG_TYPE_DEFAULT) || (mDragType == FRImageDraggerType.DRAG_TYPE_WX && diffY > 0)) {
                            mFRImageDragger.bindScaleImageView(this);
                            mFRImageDragger.onReady(getWidth() != 0 ? getWidth() : mDefWidth, getHeight() != 0 ? getHeight() : mDefHeight);
                            isIntercept = true;
                        }
                    }
                }
    ...
    private void onActionMove(MotionEvent event) {
        // 拖拽图片，只有一个触摸点时触发
        if (event.getPointerCount() == 1 && getScale() <= 1f && isImageDragging && mFRImageDragger != null) {
            mFRImageDragger.onDragging(mDownX, mDownY, event.getX(), event.getY());
        }
        mDownX = event.getX();
        mDownY = event.getY();
    }
    ...
    private void onActionUp(MotionEvent event) {
        // 释放图片
        if (getScale() <= 1f && isImageDragging && mFRImageDragger != null) {
            mFRImageDragger.onRelease();
        }
        mDownX = 0;
        mDownY = 0;
    }
     ...
     * 开启预览
     */
    public void start(FRTransitionCallback callback) {
     ...
     * 关闭预览
     */
    public void cancel(FRTransitionCallback callback) {
    ...
    public void setViewData(FRViewData data) {
        this.mViewData = data;
    }

    public FRViewData getViewData() {
        return mViewData;
    }
    ...
    public void setImageDraggerType(@FRImageDraggerType int type) {
        setImageDraggerType(type, null, getBackground());
    }

    public void setImageDraggerType(@FRImageDraggerType int type, FRImageViewerAttacher attacher, Drawable background) {
        mDragType = type;
        if (mDragType == FRImageDraggerType.DRAG_TYPE_DEFAULT) {
            mFRImageDragger = new FRDefaultFRImageDragger();
        } else if (mDragType == FRImageDraggerType.DRAG_TYPE_WX) {
            mFRImageDragger = new FRWXImageDragger();
        }
        if (mFRImageDragger != null) {
            mFRImageDragger.setBackground(background);
            if (attacher != null) mFRImageDragger.bindImageViewerAttacher(attacher);
            mFRImageDragger.setImageDraggerStateListener(mStateListener);
        }
    }

    public void clearImageDragger() {
        if (mFRImageDragger != null) {
            mFRImageDragger = null;
        }
    }
    ...
    public void showProgess() {
        if (progressView == null) {
            FRCircleProgressBar progressBar = new FRCircleProgressBar(getContext(), 0x77FAFAfA);
     ...
     * 执行进场动画
     */
    private void performEnterAnim(final FRTransitionCallback callback) {
        isImageAnimRunning = true;
        // 图片预览界面的宽高
     ...
     * 执行退场动画
     */
    private void performExitAnim(final FRTransitionCallback callback) {
